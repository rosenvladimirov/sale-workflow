...
class ProductTemplate(models.Model):
    ...
    def get_margin_classification(self):
        for template in self:
            if not template.margin_classification_id:
                if template.categ_id:
                    return template.categ_id.get_margin_classification()
            else:
                return template.margin_classification_id
        return False
        ...
        for template in self:
            ...
            if template.product_variant_count > 1:
                for product in template.product_variant_ids:
                    standard_price_ids.append(product.stock_value)
                standard_price = mean(standard_price_ids)
