...
class SaleOrderLine(models.Model):
    ...
    customer_po_ids = fields.One2many('sale.order.customer', 'sale_order_line_id', string='Customer PO ref.')
...
class SaleOrderCustomer(models.Model):
    ...
    sale_order_line_id = fields.Many2one('sale.oder.line')
    date_po = fields.Datetime('Date PO')
    name = fields.Char('Name PO')
    description = fields.Note('Description')
