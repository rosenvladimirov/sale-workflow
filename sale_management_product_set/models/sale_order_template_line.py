...
class SaleOrderTemplateLine(models.Model):
    ...
    _inherit = ['sale.order.template.line', 'product.set.mixin']

    product_set_section_id = fields.Many2one('sale.order.template.line', string='Sale Product Set Section')
    ...
    def create(self, vals_list):
        if not self._context.get('create_new_set'):
            ...
            for sequence_inx, vals in enumerate(vals_list):
                sale_order_template_id = self.env['sale.order.template'].search([('id', '=', vals['sale_order_template_id'])], limit=1)
                if sale_order_template_id.sale_order_template_line_ids:
                    last_section_id = sale_order_template_id.sale_order_template_line_ids[-1]
    ...
    def write(self, values):
        if not self._context.get('create_new_set') and values.get('product_set_id'):
            for record in self:
                order_id = record.sale_order_template_id
                sequences = {}
                line_section = {}
                sequence_inx_insert = 0
                for sequence_inx, line in enumerate(order_id.sale_order_template_line_ids.\
                ...
                for line, sequence_inx in line_section.items():
                    ...
                    'sale_order_template_id': order_id.id,
                    'sequence': sequence_inx,
                })
    ...
    def unlink(self):
        for record in self:
            ...
            for order_id in record.mapped('sale_order_template_id'):
                order_lines = order_id.sale_order_template_line_ids. \
