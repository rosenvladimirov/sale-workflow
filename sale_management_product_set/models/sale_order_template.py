...
class SaleOrderTemplate(models.Model):
    ...
    product_set_ids = fields.Many2many('sale.template.product.set',
    ...
    def _compute_product_set_ids(self):
        ...
        for record in self:
            ...
            order_lines = record.sale_order_template_line_ids.filtered(lambda r: r.product_set_id)
            ...
            for product_set_id, total_quantity in product_set_ids.items():
                product_set_ids = self.env['sale.template.product.set'].search([
                    ('sale_order_template_id', '=', record.id),
                ...
                if product_set_ids:
                    ...
                    product_set_ids.with_context(**dict(self._context, create_new_set=True)).write(
                        record.product_set_ids._get_sale_template_product_set_value(
                            record,
                            product_set_id,
                    ...
                    record.product_set_ids |= self.env['sale.template.product.set'].\
    ...
    def action_set_sections(self):
        compensation_product_id = self.with_company(self.env.company). \
            env.ref('product_set.compensation_product', raise_if_not_found=False).id
        for record in self:
            record.sale_order_template_line_ids.\
                filtered(lambda r: r.display_type == 'line_section').\
                with_context(**dict(self._context, create_new_set=True)).unlink()
            self.env['sale.template.product.set'].\
                search([('sale_order_template_id', '=', record.id)]).\
                with_context(**dict(self._context, create_new_set=True)).unlink()
            values = {}
            total_quantity = {}
            ...
            lines = record.sale_order_template_line_ids.sorted(lambda r: r.product_set_id and r.product_set_id.id or 0)
            ...
            for line in record.sale_order_template_line_ids:
                if values.get(f'section_{line.id}') and not values.get(line.product_set_id):
                    ...
                    values[line.product_set_id] = {
                        'product_set_section_id': record.\
                            with_context(**dict(self._context, create_new_set=True)).sale_order_template_line_ids.create(values[f'section_{line.id}']).id
                    }
